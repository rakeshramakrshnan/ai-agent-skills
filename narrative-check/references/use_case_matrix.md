...
- §2 documents the two layer interfaces. Surface-pass instructions are included inline;
  word-level instructions come from `word_level_signals.md`.
...
The separate `narrative-humanize` skill provides the surface pass. The structural and surface stages remain distinct.

#### 2.1.1 Which protocol steps run

**All of them, 0 through 8**, as the surface-pass protocol requires:
