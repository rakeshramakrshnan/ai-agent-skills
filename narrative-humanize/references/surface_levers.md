# Surface levers — the word-level rewrite pass

**Status:** vendored third-party content, merged into this skill.

> **Attribution and licence.** This file is adapted from the `humanize` skill by
> Harshaneel Gokhale (<https://github.com/harshaneel/humanize>), used and modified under
> the MIT Licence. The full licence text is in `LICENSE.txt` beside this skill's
> `SKILL.md`. Adaptations: YAML frontmatter removed, self-references repointed at this
> file, and the scope note below added. The nine levers, the hard rules, the rewrite
> protocol, the domain calibrations and the banned word list are the original author's
> work and are reproduced substantially unchanged.

## What this layer is, and how it relates to the structural pass

`narrative-humanize`'s own protocol does the **structural** work: what the text does —
scene handling, causal chaining, disclosure order, the moves in `interventions.md`. This
file is the **surface** pass that follows it: word choice, sentence-length variance,
hedges, punctuation, transitions, register voice.

Run the structural pass first, then this one. Running this first sands off the surface
evidence the structural pass needs in order to locate what to move.

**The register must be stated when this pass runs.** Two rules here — hard rule 2 on
semicolons and Lever 8's punctuation work — carve out an explicitly formal or academic
register, and that carve-out only fires if the register is named. On CT-04, CT-05 and
CT-06 output the register string is `formal academic prose`; the mapping for every row is
in `SKILL.md`. Without it, the default banned-vocabulary list strips conventions that are
correct, sometimes required, in that register.

**Never applied to a `REFUTED` cell.** See `interventions.md`: a cell that separates
opposite to its card is not moved at all, and no surface pass is run to "finish" it.

---


## The nine levers and the rewrite protocol

Transforms AI-generated or flat text into more natural prose using the rules below.
...
The levers below are the write-side counterparts of the signals the word-level layer grades (A–I):
...
se ("Base Models Look Human"; details in published detection literature). What gets flagged is the
...
When stakes warrant the cost, layer these on (sources: published detection literature):
...
published detection literature).
