# Word-level signals — the surface layer

**Status:** vendored third-party content, merged into this skill.

> **Attribution and licence.** This file is adapted from the `ai-check` skill by
...
> file, and the scope note below added. The nine signal categories, their severity
> mapping, the double-counting policy, the mixed-authorship overlay and the scoring
> thresholds are the original author's work and are reproduced substantially unchanged.

## What this layer is, and what it is not

This is the **word-level / surface** layer: vocabulary, sentence-length variance, hedge
density, punctuation, transitions, register, rhetorical scaffolding. `narrative-check`'s own
protocol rates the **discourse-level** layer — narrative structure, scene handling, causal
chaining — which these signals do not measure.

The two layers are reported as **separate blocks and are never merged into one number**. A
text can be clean here and fire heavily on the discourse layer; that combination is the
primary case this skill exists for (UC-11). Neither layer's score is a detector accuracy,
and a clean score here is not evidence about authorship.

Scores from this file never enter the STRUCTURAL LEAN counts, the NC-id DIFF, or any
aggregate built from matrix cells (`content_type_matrix.md` §0.8 forbids it).

---


## Forensic signal battery
